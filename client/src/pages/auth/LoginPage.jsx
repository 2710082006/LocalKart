...
export default function LoginPage() {
  ...
  const onSubmit = async (data) => {
    ...
    if (login.fulfilled.match(result)) {
      ...
      const dashMap = { customer: '/dashboard', farmer: '/farmer/dashboard', delivery: '/delivery/dashboard', admin: '/admin/dashboard' };
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-sky-600 via-sky-700 to-emerald-700 relative overflow-hidden items-center justify-center p-12">
        ...
        <div className="relative text-white max-w-md">
          <Link to="/" className="flex items-center gap-2 mb-10">
            ...
            <span className="text-2xl font-bold">Farm<span className="text-emerald-300">2</span>Door</span>
          </Link>
          <h2 className="text-4xl font-bold mb-4 leading-tight">Welcome Back to Fresh Living</h2>
          <p className="text-sky-100 text-lg leading-relaxed">Access farm-fresh produce from local farmers delivered to your doorstep in under 2 hours.</p>
          <div className="mt-10 grid grid-cols-3 gap-4">
            {[{ n: '2,500+', l: 'Farmers' }, { n: '50K+', l: 'Customers' }, { n: '98%', l: 'Satisfaction' }].map((s, i) => (
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                <Link to="/forgot-password" className="text-sm text-sky-600 hover:text-sky-700 font-medium">Forgot?</Link>
              </div>
              <div className="relative">
            ...
            <button type="submit" disabled={loading} className="btn-primary w-full !py-3.5 !text-base" id="login-submit">
              {loading ? <span className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <>Sign In <ArrowRight className="w-5 h-5" /></>}
            </button>
          </form>

          <p className="text-center text-sm text-neutral-500 mt-8">
            Don't have an account?{' '}
            <Link to="/signup" className="text-sky-600 font-semibold hover:text-sky-700">Create account</Link>
          </p>
        </motion.div>
      </div>
    </div>
